...
/** Parse `ffmpeg -list_devices true -f dshow -i dummy` stderr for the first audio capture device. */
export function parseDshowAudioDevice(listing: string): string | null {
  const match = listing.match(/"([^"]+)"\s+\(audio\)/);
  return match?.[1] ?? null;
}

function detectDshowAudioDevice(): string | null {
  const result = spawnSync("ffmpeg", ["-hide_banner", "-list_devices", "true", "-f", "dshow", "-i", "dummy"], {
    timeout: 5000,
  });
  return parseDshowAudioDevice(`${result.stderr ?? ""}`);
}

export function chooseWindowsRecorderCommand(
  hasCommand: (command: string) => boolean,
  outputPath: string,
  device: string | null = process.env.EXOCORTEX_VOICE_DSHOW_DEVICE || null,
): RecorderCommand | null {
  if (!hasCommand("ffmpeg")) return null;
  const audioDevice = device ?? detectDshowAudioDevice();
  if (!audioDevice) return null;
  // No -nostdin: ffmpeg on Windows is stopped gracefully by writing "q" to stdin
  // (kill() would hard-terminate and leave a corrupt WAV header).
  return {
    command: "ffmpeg",
    args: [
      "-hide_banner",
...
export function chooseRecorderCommand(
  ...
  if (platform === "win32") return chooseWindowsRecorderCommand(hasCommand, outputPath);
  return null;
}
...
function recorderDependencyMessage(platform: NodeJS.Platform): string {
  ...
  if (platform === "win32") return "Voice input on Windows requires ffmpeg and a microphone (set EXOCORTEX_VOICE_DSHOW_DEVICE to pick one).";
  return `Voice input is currently only implemented for Linux, macOS, and Windows (got ${platform}).`;
}
...
export class VoiceRecorder {
  ...
  static start(): VoiceRecorder {
    ...
    const child = spawn(recorder.command, recorder.args, {
      // Windows ffmpeg is stopped by writing "q" to stdin; signals hard-kill there.
      stdio: [process.platform === "win32" ? "pipe" : "ignore", "ignore", "pipe"],
    });
