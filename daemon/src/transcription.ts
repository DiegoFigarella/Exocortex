...
import { readExocortexConfig, type TranscriptionBackend } from "@exocortex/shared/config";
...
export function activeTranscriptionBackend(): TranscriptionBackend {
  return readExocortexConfig().transcription?.backend === "openai" ? "openai" : "whisper-local";
}
...
export async function transcribeAudioBytes(
  ...
  const transcribe = activeTranscriptionBackend() === "whisper-local" ? transcribeWhisperLocal : transcribeOpenAIAudio;
  return transcribe(audioBytes, mimeType, {
